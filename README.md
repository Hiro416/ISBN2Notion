...
アプリにログイン後、登録先DatabaseのURLまたは32文字IDを入力して `Notionに接続` を押します。Notionの認可画面で対象ページまたはDatabaseへのアクセスを許可すると、この端末から登録できるようになります。

OAuth接続情報はHTTPOnly Cookieへ暗号化して保存します。実験用にDBを持たない実装です。Cookieの暗号化には `NOTION_OAUTH_COOKIE_SECRET` を使い、未設定の場合は `BOOKS_APP_PASSWORD` を使います。
